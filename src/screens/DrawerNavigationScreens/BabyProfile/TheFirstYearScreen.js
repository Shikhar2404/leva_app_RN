...
export default function TheFirstYearScreen({
  route,
  navigation,
  item,
  onClickFunction,
  ...
  const TheFirstYearrecordAPI = async () => {
    ...
    if (response.status === "SUCCESS") {
      state.selected = response.data
        ? response.data.findIndex((item) => item.is_selected) == -1
          ? 0
          : response.data.findIndex((item) => item.is_selected)
        : 0;
      setState((oldState) => ({
        ...oldState,
  ...
  const addBabyMilestoneApi = async (isCompleted) => {
    ...
    if (response.status === "SUCCESS") {
      const index = state.childMilestoneData[state.selected].data.findIndex(
        (item) => item.milestone_id == response.data.fk_milestone_id
      );
      state.childMilestoneData[state.selected].data[index].is_completed =
  ...
  const MilestoneDetailsApi = async (item) => {
    ...
    if (response.status === "SUCCESS") {
      ...
      if (response.code == apiConfigs.USER_UNSUBSCRIBE) {
        const trackEventparam = { action: "Paywall_Milestone_Screen" };
        trackEvent({ event: "Paywall_Milestone_Screen", trackEventparam });
      }
    } else {
      setState((oldState) => ({
        ...oldState,
    ...
    return (
      <TouchableWithoutFeedback
        onPress={() => {
          ...
          (state.childMilestoneData[index].is_selected = !item.is_selected),
            setState((oldState) => ({ ...oldState, selected: index }));
        }}
      >
        <View>
          <View
            style={{
              ...
              marginStart: index == 0 ? 0 : 18,
              borderBottomColor: colors.Blue,
              borderBottomWidth: state.selected == index ? 3 : 0,
              marginBottom: state.selected == index ? 0 : 3,
            }}
          >
            <Text
              style={[
      ...
      >
        <View
          style={{
            ...
            justifyContent: "center",
            alignItems: "center",
            height: deviceHeightImg,
            width: deviceWidthImg,
        ...
        >
          ...
          >
            <View
              style={{
                height: item.is_completed == 1 ? 92 : 90,
                width: item.is_completed == 1 ? 92 : 90,
                borderRadius: item.is_completed == 1 ? 92 / 2 : 90 / 2,
                ...
                alignSelf: "center",
                backgroundColor:
                  item.is_completed == 1 ? colors.lightestPink : "#E6EAF0",
              }}
            >
              <FastImageView
    ...
    <View style={stylesBackground.container}>
      ...
      <View style={styles.container}>
        ...
        <View style={{ flex: 1, alignItems: "center" }}>
          <FlatList
            data={
              state.childMilestoneData.length > 0
                ? state.childMilestoneData[state.selected].data
                : []
            }
