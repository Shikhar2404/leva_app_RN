...
import { View, Text, StyleSheet, FlatList, Modal, TouchableWithoutFeedback, BackHandler, TouchableOpacity, ScrollView, SafeAreaView, Platform } from 'react-native';
...
export default function ChildProfileScreen({ route, navigation }) {
        ...
        return (
            <View style={{ width: deviceWidth / 3, justifyContent: 'center', alignItems: 'center' }}>
                <View style={{ height: 120, }}>
                    <TouchableOpacity onPress={() => actionForMilestomeModel(item, index)}>
                        <View style={{ height: 65, width: 65, borderRadius: 65 / 2, borderColor: colors.Blue, borderWidth: item.is_completed == 1 ? 2 : 0, justifyContent: 'center', alignItems: "center", alignSelf: 'center', }}>
                            <FastImageView source={{ uri: item.thumbnail_image }} style={styles.iconimagestyle} />
                        </View>
                        {item.is_completed == 1 ?
                            <View style={{ height: 25, width: 65, position: 'absolute', bottom: -10, justifyContent: 'center', alignItems: "center", alignSelf: 'center', }}>
                                <View style={{ height: 25, width: 25, backgroundColor: colors.pink, borderRadius: 25 / 2, justifyContent: 'center', alignItems: "center", }}>
                    ...
                    <View style={{ marginTop: 12, }}>
                        <Text numberOfLines={2} style={styles.nameTextStyle}>{item.short_name}</Text>
                    </View>
                </View>
            </View>
        )
    ...
    const onShare = async (image, url) => {
        const title = state.selecteditem.name + ' is ' + state.getBabyMilstone.name + '!'
    ...
    const addBabyMilestoneApi = async (isCompleted) => {
        ...
        if (response.status === "SUCCESS") {
            const index = state.milestonedata.findIndex(item => item.milestone_id == response.data.fk_milestone_id)
    ...
    const updateSomethingStatusApi = async () => {
        ...
        if (response.status === "SUCCESS") {
            ...
            navigation.goBack()
            navigation.navigate('ArticlesDetailesScreen', { id: '593dc328-a4b2-45ad-9390-e5e903c9bdb9' })
                    ...
                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                        <FastImage
                            ...
                            <FastImage
                                ...
                                style={styles.babyimageStyle}></FastImage>
                        </FastImage>
                        <View style={{ marginLeft: 10 }}>
                            <Text style={styles.subtitleStyle}>{item.name}</Text>
                        </View>
                    </View>
                    <FastImage source={state.selectedChild === item.child_id ? importImages.SelectedRadioButtons : importImages.RadioButtons} style={{ height: 26, width: 26 }}></FastImage>
                </View>
            </TouchableOpacity>
        )
    };
    return (
        <View style={styles.container}>
                ...
                <View style={styles.TextViewcontainer}>
                    <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', height: 60 }}>
                        <TouchableOpacity style={styles.textViewStyle}
                            onPress={() => ModalCalled()} >
                            <Text style={styles.titleStyle} numberOfLines={1}>{state.selecteditem.name}</Text>
                            {state.selecteditem.name ? <FastImage source={importImages.dropDownwhiteIcons} style={{ height: 32, width: 29 }}></FastImage> : null}
                        </TouchableOpacity>
                    ...
                    <Text style={styles.subtitleStyles}>{state.selecteditem.dob == '' ? state.selecteditem.age_text : state.selecteditem.age}</Text>
                </View>
                ...
                <View style={{ flex: 1, }} >
                    ...
                    >
                        <View style={{ marginBottom: 140 }}>
                            <View style={{ flex: 1, }}>
                                <Text style={styles.textbabyStyle}>{state.selecteditem.name ? state.selecteditem.dob ? "Baby’s Milestones" : 'Baby’s Development' : ''}</Text>
                                ...
                                <View style={styles.imageiconStyle}>
                                    {state.selecteditem.dob ?
                                        ...
                                        :
                                        state.selecteditem.name && state.selecteditem.dob == '' ?
                                            <View style={{ width: deviceWidth - 50, alignSelf: 'center', backgroundColor: colors.White, borderRadius: 10, borderWidth: 1, borderColor: '#E9E9E9', flex: 1, paddingHorizontal: 20, paddingVertical: 20, alignItems: 'flex-start', justifyContent: 'center', }}>
                                                <View style={{ justifyContent: 'center', alignItems: 'flex-start', }} >
                                                    <Text style={{ fontFamily: fonts.rubikRegular, fontSize: 16, color: '#00172E', }}>{state.selecteditem.content}</Text>
                                                </View>
                                            </View>
                                            : null
                                    }
                            ...
                            {state.selecteditem.name ?
                                <View style={{ width: deviceWidth - 50, alignSelf: 'center', backgroundColor: colors.White, borderRadius: 10, borderWidth: 1, borderColor: '#E9E9E9', flex: 1, marginTop: 25, paddingHorizontal: 20, paddingVertical: 20, alignItems: 'flex-start', justifyContent: 'space-between', flexDirection: 'row', }}>
                                    <View style={{ width: '90%', }}>
                                        <View style={{ flexDirection: 'row', }}>
                                            <Text style={{ fontFamily: fonts.rubikBold, fontSize: 19, color: colors.Blue, marginEnd: 15 }}>{state.selecteditem.name}</Text>

                                        </View>
                                        {state.selecteditem.due_date ?
                                            <View style={{}}>
                                                <View style={{ flexDirection: 'row', marginTop: 15, alignItems: 'center' }}>
                                                    <FastImage source={state.selecteditem.dob ? importImages.birthicon : importImages.babybornicon} style={{ height: 16, width: 12 }}></FastImage>
                                                    <Text style={{ fontFamily: fonts.rubikRegular, fontSize: 16, color: 'rgba(127, 138, 150, 1)', marginStart: 10 }}>{state.selecteditem.dob ? moment(state.selecteditem.dob).format('MM/DD/YYYY') : moment(state.selecteditem.due_date).format('MM/DD/YYYY')}</Text>
                                                </View>
                                                {state.selecteditem.dob ?
                                                    <View style={{}}>
                                                        {state.selecteditem.gender || state.selecteditem.lbs || state.selecteditem.ounces ||
                                                            state.selecteditem.inches || state.selecteditem.blood_type || state.selecteditem.nutrition
                                                            ? <View style={{ height: 20, width: deviceWidth }}></View> : null}
                                                        {state.selecteditem.gender ?
                                                            <View style={{ width: deviceWidth - 100, justifyContent: 'space-between', alignItems: 'center', paddingTop: 15, paddingBottom: 15, borderTopWidth: 0.5, borderColor: 'rgba(127, 138, 150, 1)', flexDirection: 'row', }} >
                                                                <Text style={{ fontFamily: fonts.rubikSemiBold, fontSize: 16, color: 'rgba(0, 23, 46, 1)', }}>{'Gender'}</Text>
                                                                <Text style={{ fontFamily: fonts.rubikRegular, fontSize: 16, color: 'rgba(127, 138, 150, 1)', opacity: 50 }}>{state.selecteditem.gender == 1 ? 'Male' : 'Female'}</Text>
                                                            </View>
                                                            : null}
                                                        {state.selecteditem.lbs || state.selecteditem.ounces ?
                                                            <View style={{ width: deviceWidth - 100, justifyContent: 'space-between', alignItems: 'center', paddingTop: 15, paddingBottom: 15, borderTopWidth: 0.5, borderColor: 'rgba(127, 138, 150, 1)', flexDirection: 'row', }} >
                                                                <Text style={{ fontFamily: fonts.rubikSemiBold, fontSize: 16, color: 'rgba(0, 23, 46, 1)', }}>{'Weight'}</Text>
                                                                <Text style={{ fontFamily: fonts.rubikRegular, fontSize: 16, color: 'rgba(127, 138, 150, 1)', opacity: 50 }}>{state.selecteditem.lbs != '' ? state.selecteditem.lbs + ' lb ' : ''}{state.selecteditem.ounces != '' ? state.selecteditem.ounces + ' oz' : ''}</Text>
                                                            </View>
                                                            : null}
                                                        {state.selecteditem.inches ?
                                                            <View style={{ width: deviceWidth - 100, justifyContent: 'space-between', alignItems: 'center', paddingTop: 15, paddingBottom: 15, borderTopWidth: 0.5, borderColor: 'rgba(127, 138, 150, 1)', flexDirection: 'row', }} >
                                                                <Text style={{ fontFamily: fonts.rubikSemiBold, fontSize: 16, color: 'rgba(0, 23, 46, 1)', }}>{'Height'}</Text>
                                                                <Text style={{ fontFamily: fonts.rubikRegular, fontSize: 16, color: 'rgba(127, 138, 150, 1)', opacity: 50 }}>{state.selecteditem.inches + ' in'}</Text>
                                                            </View>
                                                            : null}
                                                        {state.selecteditem.blood_type ?
                                                            <View style={{ width: deviceWidth - 100, justifyContent: 'space-between', alignItems: 'center', paddingTop: 15, paddingBottom: 15, borderTopWidth: 0.5, borderColor: 'rgba(127, 138, 150, 1)', flexDirection: 'row', }} >
                                                                <Text style={{ fontFamily: fonts.rubikSemiBold, fontSize: 16, color: 'rgba(0, 23, 46, 1)', }}>{'Blood Group'}</Text>
                                                                <Text style={{ fontFamily: fonts.rubikRegular, fontSize: 16, color: 'rgba(127, 138, 150, 1)', opacity: 50 }}>{state.bloodtypeData[Number(state.selecteditem.blood_type) - 1].label}</Text>
                                                            </View>
                                                            : null}
                                                        {state.selecteditem.nutrition ?
                                                            <View style={{ width: deviceWidth - 100, justifyContent: 'space-between', alignItems: 'center', paddingTop: 15, borderTopWidth: 0.5, borderColor: 'rgba(127, 138, 150, 1)', flexDirection: 'row', }} >
                                                                <Text style={{ fontFamily: fonts.rubikSemiBold, fontSize: 16, color: 'rgba(0, 23, 46, 1)', }}>{'Nutrition'}</Text>
                                                                <Text style={{ fontFamily: fonts.rubikRegular, fontSize: 16, color: 'rgba(127, 138, 150, 1)', opacity: 50 }}>{state.nutritionData[Number(state.selecteditem.nutrition) - 1].label}</Text>
                                                            </View>
                                                            : null}
                                                    </View>
                                                    : null}
                                    ...
                                    <TouchableWithoutFeedback onPress={() => {
                                        ...
                                        trackEvent({ event: 'Child_Profile', trackEventparam })
                                        navigation.navigate('EditChildProfileScreen', { childData: state.selecteditem, onBackRefresh: () => onBackRefresh(), isbdate: false })
                                    }}>
                                ...
                                : null}
                            {state.selecteditem.dob && (state.selecteditem.allergies || state.selecteditem.medicalconcern) ?
                                <View style={{ width: deviceWidth - 50, alignSelf: 'center', backgroundColor: colors.White, borderRadius: 10, borderWidth: 1, borderColor: '#E9E9E9', flex: 1, marginTop: 10, paddingHorizontal: 20, paddingVertical: 20, alignItems: 'flex-start', justifyContent: 'center', }}>
                                    {state.selecteditem.allergies ?
                                        <View style={{ width: deviceWidth - 100, justifyContent: 'center', alignItems: 'flex-start', paddingBottom: state.selecteditem.medicalconcern ? 15 : 0, }} >
                                            <Text style={{ fontFamily: fonts.rubikSemiBold, fontSize: 16, color: 'rgba(0, 23, 46, 1)', }}>{'Allergies'}</Text>
                                            <Text style={{ fontFamily: fonts.rubikRegular, fontSize: 16, color: 'rgba(127, 138, 150, 1)', opacity: 50, marginTop: 10 }}>{state.selecteditem.allergies}</Text>
                                        </View>
                                        : null}
                                    {state.selecteditem.medical_concerns ?
                                        <View style={{ width: deviceWidth - 100, justifyContent: 'center', alignItems: 'flex-start', paddingTop: 15, borderTopWidth: 0.5, borderColor: 'rgba(127, 138, 150, 1)', }} >
                                            <Text style={{ fontFamily: fonts.rubikSemiBold, fontSize: 16, color: 'rgba(0, 23, 46, 1)', }}>{'Concerns'}</Text>
                                            <Text style={{ fontFamily: fonts.rubikRegular, fontSize: 16, color: 'rgba(127, 138, 150, 1)', opacity: 50, marginTop: 10 }}>{state.selecteditem.medical_concerns}</Text>
                                        </View>
                                        : null}
                                </View>
                                : null}
                            ...
                            {state.selecteditem.dob == '' ?
                                <View style={{ width: deviceWidth - 50, alignSelf: 'center', marginTop: 42, justifyContent: 'center', alignItems: 'center', marginBottom: 20 }}>
                                    <TouchableWithoutFeedback onPress={() => {
                                        setState(oldState => ({ ...oldState, ischeckSomething: true }))
                                    }}>
                                        <View style={{ borderBottomColor: colors.Blue, borderBottomWidth: 1 }}>
                                            <Text style={{ fontFamily: fonts.rubikBold, fontSize: 15, color: colors.Blue, marginBottom: 2 }}>{"Something has changed"}</Text>
                                        </View>
                                    </TouchableWithoutFeedback>
                    ...
                    {state.selecteditem.dob ?
                        <BottomButton text={'Track Growth'} onPress={() => {
                            const trackEventparam = { action: 'Track Growth' }
                            trackEvent({ event: 'Child_Profile', trackEventparam })
                            navigation.navigate('TotalGrowthScreen', { child_id: state.selecteditem.child_id })
                        }} container={{ position: 'absolute', bottom: 50, backgroundColor: colors.textinputBackground, borderWidth: 1, borderColor: colors.Blue, width: deviceWidth - 50, alignSelf: 'center' }} textstyle={{ color: colors.Blue }} />
                        : null}
                    {state.selecteditem.dob ?
                        <BottomButton text={'See Milestones'} onPress={() => {
                            const trackEventparam = { action: 'See Milestones' }
                            trackEvent({ event: 'Child_Profile', trackEventparam })
                            navigation.navigate('TheFirstYearScreen', { childData: state.selecteditem })
                        }} container={{ position: 'absolute', bottom: -20, width: deviceWidth - 50, alignSelf: 'center', }} />
                        : null}
                </View>
                <TouchableOpacity style={{ position: 'absolute', top: 190 / 2, alignSelf: 'center', justifyContent: 'center', alignItems: "center", }} disabled={true}>
                    <FastImage source={importImages.ProfileIcon} style={styles.ImportImageStyle}></FastImage>
                    <View style={{ position: 'absolute', bottom: 0, top: 0, justifyContent: 'center', alignItems: 'center' }}>
                        <FastImage
                            source={importImages.appicon}
                            style={{ height: 145, width: 145, borderRadius: 145 / 2, alignSelf: 'center', }}>
                            <FastImage
            ...
            <Modal
                ...
                <TouchableWithoutFeedback onPress={() => setState(oldState => ({
                    ...
                    <View style={[styles.styles1,]}>
                        <TouchableWithoutFeedback onPress={() => setState(oldState => ({ ...oldState, childModalVisible: true }))}>
                            <View style={[styles.containerstyle, { height: state.childListData.length > 2 ? '70%' : '30%' }]}>
            ...
            <ModalView
                ...
                close={() => { setState(oldState => ({ ...oldState, ischeckSomething: false })) }}
                components={
                    <View >
                        <View style={{ width: deviceWidth - 50, }}>
                            <Text style={{ fontFamily: fonts.rubikBold, fontSize: 20, color: '#323F4B', textAlign: 'center' }}>{ }</Text>
                            <TouchableWithoutFeedback onPress={() => { setState(oldState => ({ ...oldState, ischeckSomething: false })) }}>
                        ...
                        <View style={{ marginTop: 20 }}>
                            <TouchableWithoutFeedback onPress={() => {
                                ...
                                trackEvent({ event: 'Something_has_changed', trackEventparam })
                                setState(oldState => ({ ...oldState, ischeckSomething: false })), navigation.navigate('EditChildProfileScreen', { childData: state.selecteditem, onBackRefresh: () => onBackRefresh(), isbdate: true })
                            }} >
                                <View style={[{ marginTop: 15, borderRadius: 10, height: 65, justifyContent: 'center', borderColor: 'rgba(0, 0, 0, 0.2)', borderWidth: 1, alignItems: 'center', backgroundColor: colors.textinputBackground }]}>
                                    <Text style={{ marginStart: 20, marginEnd: 20, fontFamily: fonts.rubikBold, fontSize: 15, color: colors.Blue, }}>{"Was your baby born?"}</Text>
                                </View>
                            </TouchableWithoutFeedback>
                        </View>
                        <View style={{}}>
                            <TouchableWithoutFeedback onPress={() => updateSomethingStatusApi()}>
                                <View style={[{ marginTop: 15, borderRadius: 10, height: 65, justifyContent: 'center', borderColor: 'rgba(0, 0, 0, 0.2)', borderWidth: 1, marginBottom: 20, alignItems: 'center', backgroundColor: colors.textinputBackground }]}>
                                    <Text style={{ marginStart: 20, marginEnd: 20, fontFamily: fonts.rubikBold, fontSize: 15, color: colors.Blue, }}>{"Has your pregnancy ended?"}</Text>
                                </View>
                            </TouchableWithoutFeedback>
                        </View>
