...
const subscriptionPlan = [
  "com.leva.monthlysubscription",
  "com.leva.yearlysubscription",
];
const SubscriptionScreen = ({ route, navigation }) => {
  const { connected, subscriptions, getSubscriptions, finishTransaction } =
    useIAP();
  const [state, setState] = useState({
    ...
    ListItem: [
      "Unlimited Tracking",
      "Save your Favorite Mediations/Articles/Videos",
      "View Graphs for all of your Tracking",
      "Add a Second Child",
      "View all Articles",
  ...
  const getSubscribData = async () => {
    ...
    if (purchaseData) {
      setState((oldState) => ({
        ...oldState,
        selected:
          purchaseData.productId == "com.leva.yearlysubscription" ? 1 : 0,
      }));
    }
  };
  const productId =
    state.selected == 1
      ? "com.leva.yearlysubscription"
      : "com.leva.monthlysubscription";
  const productdata = subscriptions.filter(
  ...
  useEffect(() => {
    var purchaseUpdate = purchaseUpdatedListener(async (purchase) => {
      ...
      if (receipt) {
        try {
          await finishTransaction({
            purchase,
            isConsumable: Platform.OS == "ios" ? true : false,
          });
  ...
  const action_Subscribe = () => {
    ...
    if (subscriptions.length > 0) {
      let offerTokenpass =
        Platform.OS == "android"
          ? state.strpromocodeFinalType == state.selected &&
            state.strpromocodeFinal
            ? state.offerToken
            : productdata[0].subscriptionOfferDetails[
                productdata[0].subscriptionOfferDetails.length - 1
              ].offerToken
          : "";
  ...
  const onCancelSubscriptionAction = async () => {
    ...
    navigation.navigate("WebViewScreen1", {
      url:
        Platform.OS == "ios"
          ? apiConfigs.cancelSubscriptionIOS
          : apiConfigs.cancelSubscriptionAndroid,
  ...
  const applayPromocodeCheck = () => {
    ...
    let setofferToken = "",
      setstrPrice = "";
    const isDataFound = subscriptionOfferData
      .map((item) => {
    ...
    if (isDataFound) {
      setState((oldState) => ({
        ...oldState,
        isPromoCodeModal: false,
        strpromocodeFinalType: state.selected,
        offerToken: setofferToken,
        strPrice: setstrPrice,
        strpromocodeFinal: state.strpromocode,
      }));
    } else {
      setState((oldState) => ({
        ...oldState,
        strpromocodeerror: "Please enter valid promo code",
        strpromocodeFinal: "",
      }));
    }
  };
  ...
  const androidpricelist =
    productdata && productdata.length > 0
      ? Platform.OS == "android"
        ? productdata[0].subscriptionOfferDetails[0].pricingPhases
            .pricingPhaseList
        : ""
      : "";
  const androidprice =
    productdata && productdata.length > 0 && Platform.OS == "android"
      ? state.strpromocodeFinalType == state.selected && state.strpromocodeFinal
        ? state.strPrice
        : productdata[0].subscriptionOfferDetails[0].pricingPhases
            .pricingPhaseList[androidpricelist.length - 1].formattedPrice
      : "";
  let localizedPrice =
    productdata && productdata.length > 0
      ? Platform.OS == "ios"
        ? productdata[0].localizedPrice
        : androidprice
      : "";
  var unit = localizedPrice ? localizedPrice.substring(0, 1) : "";
  var amountcheck = localizedPrice
    ? localizedPrice.replace(unit, "").trim()
    : "";
      ...
      <View style={styles.container}>
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
          ...
          <View style={{ alignItems: "center", marginTop: 30 }}>
            ...
            >
              ...
              >
                ...
                >
                  {state.selected === 0 ? "/ Monthly" : "/ Annual"}
                </Text>
              </View>
              {state.selected == 1 ? (
                <View
                ...
                >
                  <Image
                    source={importImages.subscriptionoffer}
          ...
          <View>
            <BottomButton
              text={"Subscribe"}
              onPress={() => action_Subscribe()}
              container={{ marginTop: 28, marginBottom: 21 }}
            />
            <View
              style={{
                alignSelf: "center",
            ...
            >
              <TouchableWithoutFeedback
                onPress={() => onRestorePurchaseAction()}
              >
                <View
                  style={{
              ...
              <TouchableWithoutFeedback
                onPress={() => onCancelSubscriptionAction()}
              >
                <View
                  style={{
                    borderBottomColor: colors.Blue,
            ...
            >
              <TouchableWithoutFeedback
                onPress={() => {
                  ...
                  trackEvent({
                    event: "Subscription_Options",
                    trackEventparam,
                  });
                  navigation.navigate("WebViewScreen1", {
                    url: apiConfigs.termsAndConditionUrl,
              ...
              <TouchableWithoutFeedback
                onPress={() => {
                  ...
                  navigation.navigate("WebViewScreen1", {
                    url: apiConfigs.privacyPolicyUrl,
      ...
      <ModalView
        ...
        components={
          <View>
            ...
            >
              {ConstantsText.doyouhaveaCouponCode}
            </Text>
