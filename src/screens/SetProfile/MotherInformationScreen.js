  ...
  const Action_continue = async () => {
    if (state.name.trim() === '') {
      ...
      if (response.status === "SUCCESS") {
        ...
        const trackEventparam = { name: state.name, status: state.youare == '1' ? 'A new mom' : 'An expectant mom', DOB: state.bdate, Height: feetdata + inchesdata, Weight: state.weight }
        trackEvent({ event: 'Mother_Information', trackEventparam })
        const userData = await StorageService.getItem(StorageService.STORAGE_KEYS.USER_DETAILS);
  ...
  return (
    <View style={stylesBackground.container}>
      ...
      <View style={styles.container}>
        ...
        <View>
          <Text style={styles.titleStyle} adjustsFontSizeToFit={true} numberOfLines={1}>{'Mother Information'}</Text>
          <Text style={styles.subtitleStyle}>{'Learning about you allows us to provide tailored content and support'}</Text>
        </View>
        <View style={{ flex: 1, }}>
