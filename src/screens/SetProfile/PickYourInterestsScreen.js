...
export default function PickYourInterestsScreen({ route, navigation }) {
  ...
  const Action_continue = async () => {
    ...
    const interest_id = state.pickIntersetData.filter(item => item.isSelected).map(item => item.interest_id)
    const interest_name = state.pickIntersetData.filter(item => item.isSelected).map(item => item.name)
  ...
  const renderItem = ({ item, index }) => {
    return (
      <TouchableWithoutFeedback onPress={() => OnPressItem(item, index)}>
        ...
        <View style={[item.isSelected ? styles.activeBackground : styles.inactiveBackground, { marginBottom: state.pickIntersetData.length - 1 == index ? hasNotch() ? 130 : 110 : 7 }]}>

          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginStart: 10, marginEnd: 15 }}>
            <FastImage source={{ uri: item.icon }} style={{ height: 35, width: 35, }} />
            <Text style={item.isSelected ? styles.activeText : styles.inactiveText} >{item.name}</Text>
            <FastImage source={item.isSelected ? importImages.CheckIcon : importImages.UnCheckIcon} style={{ height: 35, width: 35, }} resizeMode={'center'} />
          </View>
  ...
  return (
    <View style={stylesBackground.container}>
      ...
      <View style={styles.container}>
        <View>
          <Text style={styles.titleStyle} adjustsFontSizeToFit={true} numberOfLines={1}>{'Pick Your Interests'}</Text>
          <Text style={styles.subtitleStyle}>{'This will help us create a personalized experience for you'}</Text>
        </View>
        <FlatList
          ...
          contentContainerStyle={state.pickIntersetData.length > 0 ? {} : { flexGrow: 1, justifyContent: 'center', alignItems: 'center', }}
        />
        <BottomButton text={route.params.from === 'edit' ? 'Save' : 'Continue'} onPress={() => Action_continue()} container={{ position: 'absolute', bottom: 0 }} />
      </View>
...
const styles = StyleSheet.create({
  ...
  activeBackground: { backgroundColor: colors.Blue, borderRadius: 10, height: 60, justifyContent: 'center', },
  activeText: { fontSize: 16, fontFamily: fonts.rubikRegular, color: colors.White, width: 200, textAlign: 'center' },
  inactiveBackground: { backgroundColor: colors.White, borderRadius: 10, height: 60, justifyContent: 'center', },
  inactiveText: { fontSize: 16, fontFamily: fonts.rubikRegular, color: colors.textLable, width: 200, textAlign: 'center' },
});
