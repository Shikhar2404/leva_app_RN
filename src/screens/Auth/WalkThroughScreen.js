...
export default function WalkThroughScreen({ route, navigation }) {
  ...
  const [Imagedata, setImagedata] = useState([
    ...
    },
    {
      ...
      text: "Full community\nsupport",
      smalltext:
        "No judgment, just full support for\nwhatever you are experiencing",
  ...
  const checkScreen = async () => {
    ...
    StorageService.getItem(StorageService.STORAGE_KEYS.USER_DETAILS)
      .then(async (userData) => {
        if (userData != null) {
          if (
            userData.is_interest_selected &&
            userData.is_mother_detail_added &&
            userData.is_child_detail_added
          ) {
            if (Platform.OS == "ios") {
              const unsubscribe = await dynamicLinks().getInitialLink(
                handleDynamicLink
              );
              const unsubscribe2 = await dynamicLinks().onLink(
                handleDynamicLink
              );
              return () => {
      ...
      .then(async (userData) => {
        if (userData != null) {
          if (!userData.is_interest_selected) {
            NavigationService.resetAction("PickYourInterestsScreen", {
              from: "",
            });
  ...
  const skipbutton = () => {
    const trackEventparam = { action: Imagedata[selectedIndex].text_event };
    trackEvent({
      event: "Skip_on_page_" + (selectedIndex + 1),
      trackEventparam,
    });
    NavigationService.resetAction("SignInScreen");
  };
      ...
      {data ? (
        <View style={styles.Container}>
          ...
          <View
            style={{
              position: "absolute",
              bottom: Platform.OS === "ios" ? (hasNotch ? 41 : 30) : 30,
              flexDirection: "row",
