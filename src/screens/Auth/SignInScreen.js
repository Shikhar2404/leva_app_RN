...
import { Image, Text, View, StyleSheet, TouchableOpacity, Platform, Alert } from 'react-native';
    ...
    const SignInAPI = async (type) => {
        ...
        if (response.status === "SUCCESS") {
            if (response.data.is_new_record) {
                const trackEventparam = { action: type == '1' ? 'Email Sign In' : type == '2' ? 'Google Sign In' : type == '3' ? 'Facebook Sign In' : 'Apple Sign In' }
    ...
    return (
        <View style={stylesBackground.container}>
            ...
            <View style={styles.container}>
                ...
                <Text style={styles.welcomeTestStyle}>{"Welcome back! We're happy to see you today!"}</Text>
                ...
                <BottomButton text={'New User? Sign Up!'} onPress={() => {
