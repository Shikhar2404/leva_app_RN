...
export default function HomeScreen({ route, navigation }) {
  ...
  const AllDetailsHomeApi = async () => {
    ...
    if (response.status === "SUCCESS") {
      await StorageService.saveItem(
        "child_id",
        response.data[0].child.child_id
      );
      ...
      await StorageService.saveItem(
        "is_water_allow",
        response.data[0].child.is_water_allow
      );
  ...
  const checkSubscriptionValidate = async () => {
    NetInfo.fetch().then(async (state) => {
      if (state.isConnected) {
        ...
        const purchaseData = await StorageService.getItem(
          StorageService.STORAGE_KEYS.PURCHASE_DATA
        );
        if (purchaseData) {
  ...
  const onClickShare = async () => {
    const trackEventparam = {
      name: state.HomeDataList[0].child.name + "’s Tracking",
      action: state.HomeDataList[0].child.content,
      DOB: state.HomeDataList[0].child.age_text,
    };
  ...
  const articleFavApi = async (article_id, item, index) => {
    ...
    if (response.status === "SUCCESS") {
      if (response.code == apiConfigs.USER_UNSUBSCRIBE) {
        ...
        const trackEventparam = {
          action: "Paywall_Article_Favorite",
        };
        trackEvent({
  ...
  const videoFavApi = async (video_id, item, index) => {
    ...
    if (response.status === "SUCCESS") {
      if (response.code == apiConfigs.USER_UNSUBSCRIBE) {
        ...
        const trackEventparam = {
          action: "Paywall_Video_Favorite",
        };
        trackEvent({
          event: "Paywall_Video_Favorite",
          trackEventparam,
        });
      } else {
        const trackEventparam = { name: item.name, Like: response.data.isLike };
  ...
  const onGetPurchasesData = async () => {
    ...
    if (Purchases && Purchases.length > 0) {
      ...
      validateReceiptApi(
        Platform.OS == "android" ? Purchases[0] : sortedAvailablePurchases[0]
      );
    } else {
      validateReceiptApi();
    }
      ...
      >
        <View
          style={{
            ...
            marginEnd:
              index == state.HomeDataList[1].articles.length - 1 ? 10 : 0,
          }}
        >
          <View style={styles.listViewStyle}>
            ...
            <TouchableOpacity
              style={styles.listHeartStyle}
              onPress={() => articleFavApi(item.article_id, item, index)}
            >
              <FastImage
                source={
                  item.isLike ? importImages.heartIcon : importImages.likeIcon
                }
                style={{ height: 30, width: 30 }}
              />
            </TouchableOpacity>
            <Text style={styles.listTextStyle} numberOfLines={2}>
              {item.name}
            </Text>
          </View>
        </View>
      ...
      >
        <View
          style={{
            marginStart: index == 0 ? 10 : 0,
            marginEnd:
              index == state.HomeDataList[2].meditations.length - 1 ? 10 : 0,
          }}
        >
          <View style={styles.listViewStyle}>
            <View style={{ margin: 15 }}>
            ...
            <TouchableOpacity
              style={styles.listHeartStyle}
              onPress={() => meditationFavApi(item.meditation_id, item, index)}
            >
              <FastImage
                source={
                  item.isLike ? importImages.heartIcon : importImages.likeIcon
      ...
      >
        <View
          style={{
            marginStart: index == 0 ? 10 : 0,
            marginEnd:
              index == state.HomeDataList[3].videos.length - 1 ? 10 : 0,
          }}
        >
          <View style={styles.listViewStyle}>
            <View style={{ margin: 15 }}>
            ...
            <TouchableOpacity
              style={styles.listHeartStyle}
              onPress={() => videoFavApi(item.video_id, item, index)}
            >
              <FastImage
                source={
                  item.isLike ? importImages.heartIcon : importImages.likeIcon
        ...
        >
          ...
          >
            <View style={{ width: "25%", alignItems: "flex-end" }}>
              <FastImage
                source={{ uri: item.consultant_image }}
                style={{
                  height: 60,
                  width: 60,
                  borderRadius: 60 / 2,
  ...
  return (
    <View style={stylesBackground.container}>
      ...
      <Header
        ...
        headerTitle={
          state.HomeDataList.length > 0
            ? "Welcome, " + state.HomeDataList[0].child.user_name
            : ""
        }
        titleStyle={{
          color: colors.Blue,
          ...
          >
            {state.HomeDataList[0].child.dob ? (
              <View style={styles.serviceTabViewStyle}>
                <Text style={styles.servicesHeadingTextStyle}>
                  {state.HomeDataList[0].child.name + "’s Tracking"}
                </Text>
                ...
                <View style={styles.servicesViewStyle}>
                  <View style={styles.servicesSubViewStyle}>
                      ...
                      content={
                        <View style={{ width: "100%", marginTop: 20 }}>
                          <View style={{ marginStart: 16, marginEnd: 16 }}>
                            ...
                            >
                              {
                                "Track your nursing, including when and how long your baby feeds, when to nurse next and which breast to use first."
                              }
                            </Text>
                          </View>
                          <View
                          ...
                          >
                            ...
                            <TouchableWithoutFeedback
                              onPress={() => {
                                action_event("Explore_Nursing", "Next"),
                                  setState((oldState) => ({
                                    ...oldState,
                                    isToolTip1: true,
                                    isToolTip: false,
                    ...
                    >
                      ...
                      >
                        <FastImage
                          source={importImages.nursingHomeIcon}
                  ...
                  <View style={styles.servicesSubViewStyle}>
                      ...
                      content={
                        <View style={{ width: "100%", marginTop: 20 }}>
                          <View style={{ marginStart: 16, marginEnd: 16 }}>
                            ...
                            >
                              {
                                "Log your pumping - track the start time, duration, output, notes for pumping sessions, and review your stats on the graph."
                              }
                            </Text>
                          </View>
                          <View
                          ...
                          >
                            ...
                            >
                              <TouchableWithoutFeedback
                                onPress={() => {
                                  action_event("Explore_Pumping", "Back"),
                                    setState((oldState) => ({
                                      ...oldState,
                                      isToolTip1: false,
                                      isToolTip: true,
                            ...
                            <TouchableWithoutFeedback
                              onPress={() => {
                                action_event("Explore_Pumping", "Next"),
                                  setState((oldState) => ({
                                    ...oldState,
                                    isToolTip1: false,
                                    isToolTip2: true,
                    ...
                    >
                      ...
                      >
                        <FastImage
                          source={importImages.pumpingHomeIcon}
                  ...
                  <View style={styles.servicesSubViewStyle}>
                      ...
                      content={
                        <View style={{ width: "100%", marginTop: 20 }}>
                          <View style={{ marginStart: 16, marginEnd: 16 }}>
                            ...
                            >
                              {
                                "Record your bottle feeds, including when your baby last fed, how much they drink, and their total daily volumes."
                              }
                            </Text>
                          </View>
                          <View
                          ...
                          >
                            ...
                            >
                              <TouchableWithoutFeedback
                                onPress={() => {
                                  action_event("Explore_Bottle", "Back"),
                                    setState((oldState) => ({
                                      ...oldState,
                                      isToolTip2: false,
                                      isToolTip1: true,
                            ...
                            <TouchableWithoutFeedback
                              onPress={() => {
                                action_event("Explore_Bottle", "Next"),
                                  setState((oldState) => ({
                                    ...oldState,
                                    isToolTip2: false,
                                    isToolTip3: true,
                    ...
                    >
                      ...
                      >
                        <FastImage
                          source={importImages.bottleHomeIcon}
                          style={{ height: 58, width: 58 }}
                        ></FastImage>
                  ...
                  <View style={styles.servicesSubViewStyle}>
                      ...
                      content={
                        <View style={{ width: "100%", marginTop: 20 }}>
                          <View style={{ marginStart: 16, marginEnd: 16 }}>
                            ...
                            >
                              {
                                "Track your baby's diapers, including how many wet and dirty diapers they produce a day, and the color and consistency of their stools."
                              }
                            </Text>
                          </View>
                          <View
                          ...
                          >
                            ...
                            >
                              <TouchableWithoutFeedback
                                onPress={() => {
                                  action_event("Explore_Diaper", "Back"),
                                    setState((oldState) => ({
                                      ...oldState,
                                      isToolTip3: false,
                                      isToolTip2: true,
                            ...
                            <TouchableWithoutFeedback
                              onPress={() => {
                                action_event("Explore_Diaper", "Finish"),
                                  setState((oldState) => ({
                                    ...oldState,
                                    isToolTip3: false,
                                  }));
                    ...
                    >
                      ...
                      >
                        <FastImage
                          source={importImages.diaperHomeIcon}
                          style={{ height: 58, width: 58 }}
                        ></FastImage>
              ...
              <View style={styles.serviceTabViewStyle}>
                ...
                <Text style={styles.servicessubHeadingTextStyle}>
                  {state.HomeDataList[0].child.age_text}
                </Text>
                <View
                  style={{
                    height: 1,
                ...
                >
                  ...
                  >
                    {state.HomeDataList[0].child.content}
                  </Text>
                  <TouchableWithoutFeedback onPress={() => onClickShare()}>
                    <View>
                      <FastImage
                        source={importImages.shareiconHome}
                        style={{ height: 34, width: 34, marginEnd: -10 }}
                      ></FastImage>
                    </View>
                  </TouchableWithoutFeedback>
                </View>
      ...
      <Modal
        ...
        transparent={true}
        onRequestClose={() =>
          setState((oldState) => ({ ...oldState, charthome: "" }))
        }
      >
        <TouchableWithoutFeedback
          onPress={() =>
                ...
                <View style={[styles.mcontainerstyle]}>
                  ...
                  >
                    <View style={{ height: 40, width: 40, borderRadius: 20 }}>
                      <FastImage
                        source={importImages.successiconchart}
                        style={{ height: 40, width: 40 }}
                      ></FastImage>
                    </View>
                    <Text
                    ...
                    <TouchableWithoutFeedback
                      onPress={() => {
                        ...
                        setState((oldState) => ({
                          ...
                          navigation.navigate(state.charthome);
                      }}
                    >
                      <View
                        style={{
      ...
      !state.isToolTip1 &&
      !state.isToolTip2 &&
      !state.isToolTip3 ? null : (
        <View
          style={[
            stylesBackground.backgroundimgcontainer,
