...
export default function NursingScreen({ route, navigation }) {
  ...
  const nursingDetails = async () => {
    ...
    if (left != null) {
      if (left.status) {
        var timetwocurrentdateduration =
          new Date().getTime() -
          new Date(Math.floor(new Date(left.currentDatetime).getTime()));
    ...
    if (right != null) {
      if (right.status) {
        var timetwocurrentdateduration =
          new Date().getTime() -
          new Date(Math.floor(new Date(right.currentDatetime).getTime()));
  ...
  const getFormattedTimeLeft = async (time, duration) => {
    ...
    if (time != "00:00:00") {
      const obj = {
        duration: duration,
        status: state.isStartleft,
        currentDatetime: Math.floor(new Date().getTime()),
      };
  ...
  const Savetracking = async (type) => {
    ...
    var nursing_type =
      state.lefttime && state.righttime
        ? "3"
        : state.lefttime
        ? "1"
        : state.righttime
    ...
    if (nursing_type) {
      ...
      if (response.status === "SUCCESS") {
        ...
        if (response.code == apiConfigs.USER_UNSUBSCRIBE) {
          const trackEventparam = { action: "Paywall_Nursing_Screen" };
          trackEvent({ event: "Paywall_Nursing_Screen", trackEventparam });
        }

        if (response.code != apiConfigs.USER_UNSUBSCRIBE) {
          const trackEventparam = {
            Left_time: JSFunctionUtils.formatTimeString(
              state.lefttime,
              false,
              true
            ),
            Right_time: JSFunctionUtils.formatTimeString(
              state.righttime,
              false,
              true
            ),
            name:
              state.selectedMedi.length > 0 ? state.selectedMedi[0].title : "",
          };
  ...
  const MeditationApi = async (values) => {
    const getallComplete =
      state.LastRecored == state.meditationList.length ? false : true;
    ...
    if (response.status === "SUCCESS") {
      ...
      var meditationDatabyliked = meditationData.sort(function (x, y) {
        return x.interest_name === y.interest_name
          ? 0
          : x.interest_name === "Breastfeeding"
          ? -1
          : 1;
      });
      state.meditationList = meditationDatabyliked.sort(function (x, y) {
        return x.isLike === y.isLike ? 0 : x.isLike ? -1 : 1;
      });
      setState((oldState) => ({
        ...oldState,
        ...
        isModalVisible: false,
        pagenumber:
          response.data.total_records === state.meditationList.length
            ? state.pagenumber
            : state.pagenumber + 1,
  ...
  const Action_Missing_continue = () => {
    navigation.navigate("EditNursingScreen1", {
      Data: undefined,
      from: "tracking",
      onGoBack: (data, isnew, deleteid) => {},
    });
  };
  const bothclick = async () => {
    ...
    setState((oldState) => ({
      ...oldState,
      isStartright: !state.isStartright,
      isStartleft: !state.isStartleft,
    }));
  };
  const renderItem = ({ item, index }) => {
    return (
      <TouchableWithoutFeedback onPress={() => onSelectMedi(item, index)}>
        <View
          style={{
            marginBottom: state.meditationList.length - 1 == index ? 30 : 7,
          }}
        >
          <View
            style={{
  ...
  return (
    <View
      style={[stylesBackground.container, { backgroundColor: "transparent" }]}
    >
      <Animated.View
        style={[
          {
      ...
      <View style={{ flex: 1, justifyContent: "flex-end" }}>
        ...
        >
          ...
          <Text style={styles.discriptionTextStyle}>
            {"Tap the top left or right button to start the timer"}
          </Text>
            ...
            <View style={styles.container}>
                  ...
                  >
                    <View style={styles.imageStyle}>
                      ...
                      >
                        {state.isStartleft ? (
                          <FastImage
                            source={importImages.nursingleftGif}
                            style={{ width: 145, height: 130 }}
                          />
                        ) : (
                          <FastImage
                            source={importImages.nursingLeftIcon}
                            style={{ width: 145, height: 130 }}
                          />
                        )}
                      </View>
                  ...
                  >
                    <Stopwatch
                      ...
                      getTime={(time, duration) =>
                        getFormattedTimeLeft(time, duration)
                      }
                    />
                    <TouchableWithoutFeedback
                      onPress={() => resetTimerDataLeft()}
                    >
                      <View style={{ height: 20, width: 20, marginStart: 5 }}>
                        <FastImage
                          source={importImages.refreshIcon}
                          style={{ height: 20, width: 20 }}
                        />
                      </View>
                    </TouchableWithoutFeedback>
                ...
                <View style={styles.leftRightButtonStyle}>
                  ...
                  >
                    <View style={styles.imageStyle}>
                      {state.isStartright ? (
                        <FastImage
                          source={importImages.nursingrightGif}
                          style={{ width: 145, height: 130 }}
                        />
                      ) : (
                        <FastImage
                          source={importImages.nursingRightIcon}
                          style={{ width: 145, height: 130 }}
                        />
                      )}
                      <View
                  ...
                  >
                    <Stopwatch
                      ...
                      getTime={(time, duration) =>
                        getFormattedTimeRight(time, duration)
                      }
                    />
                    <TouchableWithoutFeedback
                      onPress={() => resetTimerDataRight()}
                    >
                      <View style={{ height: 20, width: 20, marginStart: 5 }}>
                        <FastImage
                          source={importImages.refreshIcon}
              ...
              <View style={styles.leftRightBothView}>
                <TouchableWithoutFeedback onPress={() => bothclick()}>
                  <View>
                    <Text
                      style={[
                        state.isStartright && state.isStartleft
              ...
              {state.lastSession ? (
                <View style={styles.sessionViewStyle}>
                  <Text style={styles.sessionTextStyle}>
                    {"Your Last Session:"}
                  </Text>
                  <View style={{ marginTop: 10 }}>
                    <Text style={styles.sessionResultStyle}>
                      {state.lastSession.type === "Both"
                        ? JSFunctionUtils.formatTimeString(
                            state.lastSession.duration,
                            false,
                            true
                          ) +
                          " " +
                          state.lastSession.type +
                          " Breasts"
                        : JSFunctionUtils.formatTimeString(
                            state.lastSession.duration,
                            false,
                    ...
                    {state.lastSession.type === "Both" ? (
                      <Text style={[styles.sessionResultStyle]}>
                        {JSFunctionUtils.formatTimeString(
                          state.lastSession.left_duration,
                          false,
                          true
                        ) +
                          " Left, " +
                          JSFunctionUtils.formatTimeString(
                            state.lastSession.right_duration,
                            false,
                            true
                          ) +
                          " Right"}
              ...
              <Text style={styles.meditationTextStyle}>
                {"Meditations for Nursing"}
              </Text>

              {state.selectedMedi.length > 0 ? (
                <TouchableWithoutFeedback
                  onPress={() => click_SelectMediList()}
                >
                  <View style={styles.audioViewStyle}>
                    ...
                    >
                      <FastImage
                        style={styles.audioImageStyle}
                        source={{ uri: state.selectedMedi[0].artwork }}
                      />
                      <View style={styles.audioSubViewStyle}>
                        <Text style={styles.audioNameStyle}>
                          {state.selectedMedi[0].title}
                        </Text>
                      </View>
                    </View>
                    <View style={styles.buttonViewStyle}>
                      ...
                      >
                        <FastImage
                          source={
                            state.isPlaying
                              ? importImages.nursingTrackPauseIcon
                              : importImages.nursingTrackStartIcon
                          }
                          style={styles.startstopButtonStyle}
                        />
                      </TouchableWithoutFeedback>
                      <TouchableWithoutFeedback onPress={() => NextPlay()}>
                        <FastImage
                          source={importImages.nursingTrackSkipIcon}
                ...
                <BottomButton
                  text={"Select meditation for play"}
                  container={{ marginBottom: 0, marginTop: 15 }}
                  textstyle={{
                    fontFamily: fonts.rubikBold,
                    textTransform: "none",
                  }}
              ...
              <View
                style={{
                  ...
                  alignItems: "center",
                  justifyContent: "center",
                  marginBottom: hasNotch() ? 35 : 20,
                }}
              >
                <TouchableWithoutFeedback
                  onPress={() => Action_Missing_continue()}
                >
                  <View
                    style={{
      ...
      {state.isSubscribe ? (
        <SubscriptionModalView
          ...
          subScribeOnClick={() => {
            navigation.navigate("SubscriptionScreen1");
          }}
          onClose={() => {
            navigation.goBack();
          }}
