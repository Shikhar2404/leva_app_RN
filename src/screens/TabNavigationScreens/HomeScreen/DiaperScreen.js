...
export default function DiaperScreen({ route, navigation }) {
  const [state, setState] = useState({
    ...
    diaperTypesData: [
      {
        name: "BM",
        imageActive: importImages.bmactive,
        imageDeactive: importImages.bmdeactive,
      },
      {
        name: "Wet",
        imageActive: importImages.wetactive,
        imageDeactive: importImages.wetdeactive,
      },
      {
        name: "Dry",
        imageActive: importImages.dryactive,
        imageDeactive: importImages.drydeactive,
      },
    ],
    daiperData: [],
  ...
  var namrSC = state.stool_color_name
    ? state.stool_color_name.toLowerCase().replace(/\s/g, "")
    : "";
  ...
  const selectedRadioIndex = (index, item) => {
    if (item.is_selected) {
      state.diaper_situation_ids = [
        ...state.diaper_situation_ids.filter((data) => data != item.id),
      ];
    } else {
  ...
  const renderItem = ({ item, index }) => {
    return (
      <View>
        <TouchableOpacity
          onPress={() => selectedRadioIndex(index, item)}
          style={{
            flexDirection: "row",
            justifyContent: "space-between",
            marginTop: index == 0 ? 16 : 22,
            marginEnd: 2,
            marginBottom: 2,
          }}
        >
  ...
  const StoolRenderItem = ({ item, index }) => {
    return (
      <View style={{}}>
        <TouchableOpacity
          onPress={() => calltheIndexValue(index, item)}
          style={
            state.storeIdStoolData == item.id
        ...
        {[
          state.stoolData.length - 1 == index ? null : (
  ...
  const StoolColorRenderItem = ({ index, item }) => {
    return (
      <View style={{}}>
        <TouchableOpacity
          onPress={() => callTheIndex(index, item)}
          style={
            state.storeIdStoolColor == item.id
              ? { backgroundColor: "white" }
              : {}
          }
        >
          ...
          >
            ...
            <FastImage
              source={{ uri: item.concat }}
              style={[
                styles.StoolImageStyle,
                {
                  borderColor:
                    state.storeIdStoolColor == item.id
                      ? item.name.toLowerCase().replace(/\s/g, "") != "white"
          ...
          {[
            state.stoolColor.length - 1 == index ? null : (
              <View style={styles.line}></View>
            ),
          ]}
        </TouchableOpacity>
      ...
      <View style={{ marginTop: 20, marginHorizontal: 20 }}>
        <TouchableOpacity onPress={() => selectDipaerType(item, index)}>
          ...
          <Text
            style={[
              styles.bmtextStyle,
              state.selectedDiaper === index ? {} : { opacity: 0.5 },
            ]}
          >
            {item.name}
          </Text>
        ...
        <View style={{ width: (deviceWidth - 50) / 3 }}>
          <View style={{ height: 85, width: "100%" }}>
            <View style={{ marginEnd: index != 2 ? 7 : 0, marginTop: 20 }}>
              <TouchableOpacity
                onPress={() => managequantitysize(index)}
                style={
                  state.selectedquantitysize === index
                    ? styles.qtyActiveBtn
                    : styles.qtyInActiveBtn
                }
              >
                <Text
                  style={
    ...
    if (response.status === "SUCCESS") {
      ...
      if (response.code != apiConfigs.USER_UNSUBSCRIBE) {
        const trackEventparam = {
          Diaper_Type: state.diaperTypesData[state.selectedDiaper],
          Diaper_SubType: state.daiperData
            .filter((data) => data.is_selected)
            .map((item) => item.name)
            .toString(),
          quantity: state.qtyData[state.selectedquantitysize],
          Note: state.note,
          Stool_Consistency: state.stool_consistency_name,
          Stool_Color: state.stool_color_name,
          alert:
            namrSC == "white" ||
            namrSC == "red" ||
            (namrSC == "black" && state.isMsgTrue)
              ? "You selected a stool color that you should discuss with your pediatrician."
              : "",
        };
      ...
      if (response.code == apiConfigs.USER_UNSUBSCRIBE) {
        const trackEventparam = { action: "Paywall_Diaper_Screen" };
        trackEvent({ event: "Paywall_Diaper_Screen", trackEventparam });
      }
    } else {
  ...
  const Action_Missing_continue = () => {
    navigation.navigate("EditDiaperScreen1", {
      Data: undefined,
      from: "tracking",
                ...
                <View style={{ width: deviceWidth - 50 }}>
                  ...
                  {state.selectedDiaper != 1 ? (
                    <View>
                      ...
                      >
                        <View style={{ width: "55%" }}>
                          <Text style={styles.stoolConsisTextStyle}>
                            {"Stool Consistency:"}
                          </Text>
                        </View>
                        <View style={{ width: "45%" }}>
                      ...
                      >
                        ...
                        >
                          <TouchableOpacity
                            onPress={() => modalAndConsistencyAPI()}
                            style={[
                              state.stool_consistency_name != ""
                                ? styles.qtyActiveBtn
                                : styles.qtyInActiveBtn,
                            ]}
                          >
                        ...
                        >
                          <TouchableOpacity
                            onPress={() => modalAndColorApi()}
                            style={[
                              state.stool_color_name != ""
                                ? styles.qtyActiveBtn
                                : styles.qtyInActiveBtn,
                              {},
                            ]}
