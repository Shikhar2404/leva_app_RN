...
export default function PumpingScreen({ route, navigation }) {
    ...
    if (starttimeStorage != null) {
      if (starttimeStorage.status) {
        var timetwocurrentdateduration =
          new Date().getTime() -
          new Date(
            Math.floor(new Date(starttimeStorage.currentDatetime).getTime())
          );
  ...
  const MeditationsDetailsApi = async (data) => {
    ...
    if (response.status === "SUCCESS") {
      ...
      if (response.code == apiConfigs.USER_UNSUBSCRIBE) {
        ...
        const trackEventparam = { action: "Paywall_Pumping_Screen" };
        trackEvent({ event: "Paywall_Pumping_Screen", trackEventparam });
      }
    } else {
      if (response) {
        showSimpleAlert(response.message);
      ...
      if (response.status === "SUCCESS") {
        ...
        if (response.code != apiConfigs.USER_UNSUBSCRIBE) {
          const trackEventparam = {
            duration: JSFunctionUtils.formatTimeString(
              state.starttime,
              false,
              true
            ),
            ...
            Right_time: state.rightamt,
            Left_time: state.leftamt,
          };
  ...
  const Action_Missing_continue = () => {
    navigation.navigate("EditPumpingScreen1", {
      Data: undefined,
      from: "tracking",
  ...
  const startClick = async () => {
    ...
    if (nursinglefttime) {
      const obj = {
        duration: nursinglefttime.duration,
        status: false,
        currentDatetime: Math.floor(new Date().getTime()),
      };
    ...
    if (nursingrighttime) {
      const obj = {
        duration: nursingrighttime.duration,
        status: false,
        currentDatetime: Math.floor(new Date().getTime()),
      };
    ...
    setState((oldState) => ({
      ...oldState,
      isStarttTmer: !state.isStarttTmer,
    ...
    >
      ...
      <View style={{ flex: 1, justifyContent: "flex-end" }}>
        ...
        >
          ...
          >
            <TouchableWithoutFeedback
              onPress={() => handleSubmitEditing(state.refleft)}
            >
              <View>
                ...
                <View style={styles.leftRightButtonStyle}>
                  <TouchableWithoutFeedback
                    onPress={() =>
                      setState((oldState) => ({
                        ...oldState,
                        stopwatchStartleft: !state.stopwatchStartleft,
                      }))
                    }
                  >
                ...
                <View style={styles.leftRightButtonStyle}>
                  <TouchableWithoutFeedback
                    onPress={() =>
                      setState((oldState) => ({
                        ...oldState,
                        stopwatchStartright: !state.stopwatchStartright,
                      }))
                    }
                  >
                    <View style={styles.imageStyle}>
                  ...
                  {state.starttime != 0 || state.isPause ? (
                    <TouchableWithoutFeedback onPress={() => resetTimerData()}>
                      <View style={[styles.btnTextStyle, { marginEnd: 30 }]}>
                        <FastImage
                          source={importImages.pumpingStopIcon}
                  ...
                  {state.isPause ? (
                    ...
                    >
                      <View style={[styles.btnTextStyle]}>
                        <FastImage
                          source={importImages.pumpingPauseIcon}
                          style={{ height: 51, width: 51 }}
                        ></FastImage>
                        <Text style={styles.timerPauseTextStyle}>
                          {"pause"}
                        </Text>
                      </View>
                    </TouchableWithoutFeedback>
                    ...
                    <TouchableWithoutFeedback onPress={() => startClick()}>
                      <View style={styles.btnTextStyle}>
                        <FastImage
                          source={importImages.pumpingPlayIcon}
                          style={{ height: 51, width: 51 }}
                        ></FastImage>
                        <Text style={styles.timerPauseTextStyle}>
                          {"start timer"}
