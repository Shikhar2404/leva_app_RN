...
export default function BottleScreen({ route, navigation }) {
  ...
  useEffect(() => {
    const unsubscribe = navigation.addListener("focus", async () => {
      ...
      if (!is_water_allow) {
        setState((oldState) => ({
          ...oldState,
          nutritionData: state.nutritionData.filter(
            (item) => item.value != "1"
          ),
        }));
      }
  ...
  const bottleTrackingAPI = async () => {
    ...
    if (state.start_date_time == "") {
      ...
      let params = {
        ...
        details: state.bottleData.filter(
          (item) => item.quantity != "" && item.liquid_type != ""
        ),
      };
      ...
      if (response.status === "SUCCESS") {
        ...
        if (response.code != apiConfigs.USER_UNSUBSCRIBE) {
          let liquid_type = state.bottleData
            .filter((item) => item.quantity != "" && item.liquid_type != "")
            .map((item, index) => {
              var dataind = state.nutritionData.find(
                (items) => items.value == item.liquid_type
              );
              return dataind.label;
            })
            .toString();
          let quantity = state.bottleData
            .filter((item) => item.quantity != "" && item.liquid_type != "")
          ...
          const trackEventparam = {
            ...
            datetime: moment(state.start_date_time).format("h:mm A"),
          };
        ...
        if (response.code == apiConfigs.USER_UNSUBSCRIBE) {
          const trackEventparam = { action: "Paywall_Bottle_Screen" };
          trackEvent({ event: "Paywall_Bottle_Screen", trackEventparam });
        }
      } else {
  ...
  const Action_Missing_continue = () => {
    navigation.navigate("EditBottleScreen1", {
      Data: undefined,
      from: "tracking",
        ...
        >
          <View style={{ width: "48%" }}>
            <TextField
              ...
              isShowImg={false}
              onChangeText={(text) =>
                handleChangeOfText("quantity", index, text, item.liquid_type)
              }
              blurOnSubmit={true}
              maxLength={3}
      ...
      <View style={{ flex: 1, justifyContent: "flex-end" }}>
        ...
        >
                      ...
                      <TextField
                        ...
                        value={
                          state.start_date_time != ""
                            ? moment(state.start_date_time).format("h:mm A")
                            : ""
                        }
                        ...
                        isShowImg={true}
                        onChangeText={(text) =>
                          handleChangeOfText("start_date_time", text)
                        }
                        blurOnSubmit={true}
                        editable={false}
                        lable={""}
          ...
          <BottomButton
            ...
            textstyle={{ fontFamily: fonts.rubikBold, textTransform: "none" }}
            onPress={() => bottleTrackingAPI()}
          />
          <View
            style={{
              flexDirection: "row",
