...
export default function RecentAllBottlesScreen({ route, navigation }) {
  ...
  const sessionListApi = async (values, valuesfooter) => {
    ...
    let response = await Request.post('bottle/list', params)
    if (response.status === 'SUCCESS') {

      setState(oldState => ({
        ...oldState,
        sessionData: state.isRefresh ? response.data.bottles : JSFunctionUtils.uniqueArrayDate(state.sessionData, response.data.bottles, "id"),
        isModalVisible: false,
  ...
  const deleteApi = async (bottle_id, indexs, id) => {
    action_event('Delete')
  ...
  const addupdate = (newdata, isnew, deleteid) => {
    if (!newdata) {
      ...
      var array = [...state.sessionData];
      array[indmain].data = [...state.sessionData[indmain].data.filter((data, index) => data.bottle_id != deleteid.bottle_id)];
      if (array[indmain].data.length == 0) {
      ...
      if (isnew) {
        ...
        setState(oldState => ({
          ...oldState,
          sessionData: JSFunctionUtils.uniqueArrayDate(state.sessionData, dataarray, "id", "time"),
          LastRecored: isnew ? state.LastRecored + 1 : state.LastRecored,
        }))
      }
      else {
        var indmain = state.sessionData.findIndex(data => data.id === newdata.id)
        var indsub = indmain != -1 ? state.sessionData[indmain].data.findIndex(data => data.bottle_id === newdata.bottle_id) : -1
        if (indsub != -1) {
          state.sessionData[indmain].data[indsub] = newdata;
          state.sessionData[indmain].data = state.sessionData[indmain].data.sort((a, b) => (a.time > b.time) ? -1 : ((b.time > a.time) ? 1 : 0))
          setState(oldState => ({
            ...oldState,
          ...
          for (let i = 0; i < state.sessionData.length; i++) {
            const olddata = state.sessionData[i].data.filter((data, index) => data.bottle_id == newdata.bottle_id)
            state.sessionData[i].data = [...state.sessionData[i].data.filter((data, index) => data.bottle_id != newdata.bottle_id)];
            if (olddata.length > 0) {
  ...
  const Action_continue = () => {
    ...
    navigation.navigate(route.params.from == 'tracking' ? 'EditBottleScreen1' : 'EditBottleScreen', { Data: undefined, from: route.params.from == 'tracking' ? 'tracking' : 'recent', onGoBack: (data, isnew, deleteid) => addupdate(data, isnew, deleteid) })
  }
  const onRefresh = () => {
    state.pagenumber = 1
    state.LastRecored = 0
  ...
  const action_event = (action) => {
    ...
    trackEvent({ event: 'Recent_Bottle_Sessions', trackEventparam })
  }
    ...
    return (
      <Swipeout backgroundColor={'transparent'}
        ...
        right={[{
          onPress: () => {
            action_event('Edit')
            navigation.navigate(route.params.from == 'tracking' ? 'EditBottleScreen1' : "EditBottleScreen", { Data: item, from: route.params.from == 'tracking' ? 'tracking' : 'recent', onGoBack: (data, isnew, deleteid) => addupdate(data, isnew, deleteid) })},
        ...
        {
          onPress: () => deleteApi(item.bottle_id, index, section.id),
          backgroundColor: 'transparent',
          component: (
            <View style={{ alignItems: 'center', flex: 1, justifyContent: "center" }}>
        ...
        <View style={{ marginBottom: 7, flexDirection: 'row', width: deviceWidth - 34, justifyContent: 'space-between', alignItems: 'center', marginTop: 15 }}>
          ...
          <View style={[styles.itemBackground, { width: deviceWidth - 94, }]}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginStart: 10, marginEnd: 15 }}>
              <Text style={styles.inactiveText}>{item.liquid_name}</Text>
    ...
    <View style={stylesBackground.container}>
      ...
      <View style={{ width: deviceWidth - 34, alignItems: 'center', alignSelf: 'center' }}>
        <View style={{ width: '100%', justifyContent: 'space-between', flexDirection: 'row', alignItems: 'center' }}>
          <Text style={styles.mainHeadertext} adjustsFontSizeToFit={true} numberOfLines={1}>{'Recent Bottle Sessions'}</Text>
