  ...
  const graphDataApi = async () => {
    ...
    if (response.status === 'SUCCESS') {
      ...
      if (response.data.length > 0) {
        ...
        data.map(item => {
          const obj = item.total_hours != 0 ? { value: item.total_hours, label: item.title, topLabelComponent: () => (<Text style={{ color: colors.Blue, fontSize: 10, fontFamily: fonts.rubikBold, marginBottom: 10, width: 32, textAlign: 'center' }}>{item.total_hours}</Text>) } : { value: item.total_hours, label: item.title }
  ...
  return (
    <View style={stylesBackground.container}>
      ...
      <View style={{ width: deviceWidth - 34, alignItems: 'center', alignSelf: 'center', }}>
        <View style={{ width: '100%', justifyContent: 'space-between', flexDirection: 'row', }}>
          <View style={{ height: 45, justifyContent: 'center' }}>
            <Text style={styles.mainHeadertext} adjustsFontSizeToFit={true} numberOfLines={1}>{'Total Nursed'}</Text>
          </View>
      ...
      <View style={styles.container}>
          ...
          <View>
            <View style={{ width: deviceWidth - 34, borderBottomWidth: 1, borderTopWidth: 1, borderColor: 'rgba(34, 50, 99, 1)', height: 50, marginTop: state.isShowDay ? 3 : 48, flexDirection: 'row', alignItems: 'center' }}>
              ...
              <TouchableWithoutFeedback onPress={() => click_avg()}>
                ...
                <View style={{ marginStart: 31, }}>
                  <Text style={state.istype == '3' ? styles.activeStyle : styles.inactiveStyle}>{'Avg Time'}</Text>
                </View>
              </TouchableWithoutFeedback>
            </View>
            <ScrollView showsVerticalScrollIndicator={false} bounces={true} style={{}}>
              <View style={{ marginTop: 10, alignItems: 'center', height: deviceHeight / 1.5, marginBottom: 120 }} >
                <BarChart
                  ...
                  yAxisLabelSuffix={state.istype == '1' ? ' hr' : state.istype == '3' ? ' min' : ''}
        ...
        <BottomButton text={'See all recent nursing sessions'} onPress={() => Action_continue()} container={{ position: 'absolute', bottom: -20 }} />
