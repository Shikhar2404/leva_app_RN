    ...
    if (response.status === 'SUCCESS') {
      ...
      if (response.data.length > 0) {
        ...
        data.map(item => {
          const obj = {
            stacks: [
              { value: item.water, color: state.istype != '' ? colors.Darkpink : colors.Purple, innerBarComponent: () => (<Text style={{ color: colors.Blue, fontSize: 10, fontFamily: fonts.rubikRegular, width: 28, textAlign: 'center', position: 'absolute', bottom: 5 }}>{state.istype != '' ? '' : item.water != 0 ? item.water : ''}</Text>) },
              { value: item.formula, color: state.istype != '' ? colors.Darkpink : colors.grayDark, innerBarComponent: () => (<Text style={{ color: colors.Blue, fontSize: 10, fontFamily: fonts.rubikRegular, width: 28, textAlign: 'center', position: 'absolute', bottom: 5 }}>{state.istype != '' ? '' : item.formula != 0 ? item.formula : ''}</Text>) },
              { value: item.milk, color: state.istype != '' ? colors.Darkpink : colors.Darkpink, innerBarComponent: () => (<Text style={{ color: colors.Blue, fontSize: 10, fontFamily: fonts.rubikRegular, width: 28, textAlign: 'center', position: 'absolute', bottom: 5 }}>{state.istype != '' ? '' : item.milk != 0 ? item.milk : ''}</Text>) },
            ],
            label: item.title,
            topLabelComponent: () => (
              <View>
                <Text style={{ color: colors.Blue, fontSize: 10, fontFamily: fonts.rubikBold, marginBottom: -10, width: 28, textAlign: 'center' }}>{item.total != 0 ? item.total : ''}</Text>
              </View>
            )
          }
  ...
  return (
    <View style={stylesBackground.container}>
      ...
      <View style={{ width: deviceWidth - 34, alignItems: 'center', alignSelf: 'center' }}>
        <View style={{ width: '100%', justifyContent: 'space-between', flexDirection: 'row', alignItems: 'flex-start' }}>
          <View style={{ height: 45, justifyContent: 'center' }}>
            <Text style={styles.mainHeadertext} adjustsFontSizeToFit={true} numberOfLines={1}>{'Total Bottles'}</Text>
          </View>
      ...
      <View style={styles.container}>
        <View style={{ width: deviceWidth - 34, borderBottomWidth: 1, borderTopWidth: 1, borderColor: 'rgba(34, 50, 99, 1)', height: 50, marginTop: state.isShowDay ? 3 : 48, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
          <TouchableWithoutFeedback onPress={() => click_total()}>
            <View>
              <Text style={state.istype == '' ? styles.activeStyle : styles.inactiveStyle}>{'Total'}</Text>
            </View>
          </TouchableWithoutFeedback>
          <TouchableWithoutFeedback onPress={() => click_Milk()}>

            <View style={{}}>
              <Text style={state.istype == '3' ? styles.activeStyle : styles.inactiveStyle}>{'Milk'}</Text>
            </View>
          </TouchableWithoutFeedback>
          <TouchableWithoutFeedback onPress={() => click_Formula()}>

            <View style={{}}>
              <Text style={state.istype == '2' ? styles.activeStyle : styles.inactiveStyle}>{'Formula'}</Text>
            </View>
          </TouchableWithoutFeedback>
          <TouchableWithoutFeedback onPress={() => click_Water()}>

            <View style={{}}>
              <Text style={state.istype == '1' ? styles.activeStyle : styles.inactiveStyle}>{'Water'}</Text>
            </View>
          </TouchableWithoutFeedback>
        </View>
        {state.dataGraph.length > 0 ?
          <View style={{ flex: 1 }}>
            ...
            <ScrollView showsVerticalScrollIndicator={false} bounces={true} style={{ flex: 1, }}>
              ...
              <View style={{ marginTop: 10, alignItems: 'center', height: deviceHeight / 1.5, marginBottom: 120 }} >
                <BarChart
                ...
                />
                {state.istype == '' ?
                  <View style={{ width: deviceWidth - 50, height: 50, flexDirection: 'row', alignItems: 'center', marginTop: 50, marginStart: 74, }}>
                    <View style={{ flexDirection: 'row' }}>
                      <View style={{ height: 15.22, width: 15.22, backgroundColor: colors.Darkpink, marginEnd: 4 }}></View>
                      <Text style={{ fontFamily: fonts.rubikSemiBold, fontSize: 12, color: colors.Blue }}>{'Milk'}</Text>
                    </View>
                    <View style={{ flexDirection: 'row', marginStart: 17 }}>
                      <View style={{ height: 15.22, width: 15.22, backgroundColor: colors.grayDark, marginEnd: 4 }}></View>
                      <Text style={{ fontFamily: fonts.rubikSemiBold, fontSize: 12, color: colors.Blue }}>{'Formula'}</Text>
                    </View>
                    <View style={{ flexDirection: 'row', marginStart: 17 }}>
                      <View style={{ height: 15.22, width: 15.22, backgroundColor: colors.Purple, marginEnd: 4 }}></View>
                      <Text style={{ fontFamily: fonts.rubikSemiBold, fontSize: 12, color: colors.Blue }}>{'Water'}</Text>
                    </View>
                  </View>
                  : null}
              </View>
        ...
        <BottomButton text={'See all recent bottle sessions'} onPress={() => Action_continue()} container={{ position: 'absolute', bottom: -20 }} />
