...
export default function RecentAllPumpedScreen({ route, navigation }) {
  ...
  const sessionListApi = async (values, valuesfooter) => {
    ...
    let response = await Request.post('pumping/list', params)
    if (response.status === 'SUCCESS') {
      setState(oldState => ({
        ...oldState,
        sessionData: state.isRefresh ? response.data.pumping : JSFunctionUtils.uniqueArrayDate(state.sessionData, response.data.pumping, "id"),
        isModalVisible: false,
  ...
  const deleteApi = async (pumping_id, indexs, id) => {
    action_event('Delete')
    setState(oldState => ({
      ...oldState,
      isModalVisible: true,
  ...
  const addupdate = (newdata, isnew, deleteid) => {
    if (!newdata) {
      ...
      var array = [...state.sessionData];
      array[indmain].data = [...state.sessionData[indmain].data.filter((data, index) => data.pumping_id != deleteid.pumping_id)];
      if (array[indmain].data.length == 0) {
      ...
      if (isnew) {
        ...
        var indsub = indmain != -1 ? state.sessionData[indmain].data.findIndex(data => data.pumping_id === newdata.pumping_id) : -1
        if (indsub != -1) {
          ...
          for (let i = 0; i < state.sessionData.length; i++) {
            const olddata = state.sessionData[i].data.filter((data, index) => data.pumping_id == newdata.pumping_id)
            state.sessionData[i].data = [...state.sessionData[i].data.filter((data, index) => data.pumping_id != newdata.pumping_id)];
            if (olddata.length > 0) {
  ...
  const Action_continue = () => {
    action_event('Add Mission Session')
    navigation.navigate(route.params.from == 'tracking' ? 'EditPumpingScreen1' : 'EditPumpingScreen', { Data: undefined, from: route.params.from == 'tracking' ? 'tracking' : 'recent', onGoBack: (data, isnew, deleteid) => addupdate(data, isnew, deleteid) })
  }
  const onRefresh = () => {
    state.pagenumber = 1
    state.LastRecored = 0
  ...
  const action_event = (action) => {
    ...
    trackEvent({ event: 'Recent_Pumping_Sessions', trackEventparam })
  }
  const renderFooter = () => {
    return (
    ...
    return (
      <Swipeout backgroundColor={'transparent'}
        ...
        right={[{
          onPress: () => {
            action_event('Edit')
            navigation.navigate(route.params.from == 'tracking' ? 'EditPumpingScreen1' : 'EditPumpingScreen', { Data: item, from: route.params.from == 'tracking' ? 'tracking' : 'recent', onGoBack: (data, isnew, deleteid) => addupdate(data, isnew, deleteid) })
          },
        ...
        {
          onPress: () => deleteApi(item.pumping_id, index, section.id),
          backgroundColor: 'transparent',
          component: (
            <View style={{ alignItems: 'center', flex: 1, justifyContent: "center" }}>
        ...
        <View style={{ marginBottom: 7, flexDirection: 'row', width: deviceWidth - 34, justifyContent: 'space-between', alignItems: 'center', marginTop: 15 }}>
          ...
          <View style={[styles.itemBackground, { width: deviceWidth - 94, }]}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginStart: 10, marginEnd: 15 }}>
              <View>
                <Text style={styles.inactiveText}>{item.left_ounces != '' ? 'Left: ' + item.left_ounces + ' oz ' : ''}</Text>
                <Text style={[styles.inactiveText, { marginTop: 5 }]}>{item.right_ounces != '' ? 'Right: ' + item.right_ounces + ' oz' : ''}</Text>
              </View>
              <Text style={{ color: colors.textLable, fontFamily: fonts.rubikRegular, fontSize: 16 }}>{item.start_time}</Text>
            </View>
    ...
    <View style={stylesBackground.container}>
      ...
      <View style={{ width: deviceWidth - 34, alignItems: 'center', alignSelf: 'center' }}>
        <View style={{ width: '100%', justifyContent: 'space-between', flexDirection: 'row', alignItems: 'center' }}>
          <Text style={styles.mainHeadertext} adjustsFontSizeToFit={true} numberOfLines={1}>{'Recent Pumping Sessions'}</Text>
