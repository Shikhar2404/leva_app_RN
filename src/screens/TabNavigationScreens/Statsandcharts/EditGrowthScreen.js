...
export default function EditGrowthScreen({ route, navigation }) {
    ...
    useEffect(() => {
        if (state.data) {
            state.type = state.data.inches != '' ? 2 : state.data.head_size != '' ? 3 : 1,
                setState(oldState => ({
                    ...oldState,
                    notes: state.data.note,
                    typedata: state.type == 1 ? state.data.ounces.toString() : state.type == 2 ? state.data.inches.toString() : state.data.head_size.toString(),
                    child_id: state.data.fk_child_id,
                    growth_id: state.data.growth_id,
                    start_date: state.data.date,
                    pounds: state.type == 1 ? state.data.lbs.toString() : '',
    ...
    const deleteApi = async () => {
        ...
        if (response.status === 'SUCCESS') {
            if (state.type == 1) {
                const trackEventparam = { Weight: state.pounds + '/' + state.typedata,Select_Date: state.start_date, Note:state.notes}
                trackEvent({ event: 'Delete_Weight_Growth', trackEventparam })
            }
            else if (state.type == 2) {
                const trackEventparam = { Height: state.typedata, Select_Date: state.start_date , Note:state.notes}
                trackEvent({ event: 'Delete_Height_Growth', trackEventparam })
            }
            else {
                const trackEventparam = { Head_Size: state.typedata,  Select_Date: state.start_date , Note:state.notes}
                trackEvent({ event: 'Delete_Head_Size_Growth', trackEventparam })
            }
            showSimpleAlert(response.message)
            var data = false
            var isnew = false
            var deleteid = { growth_id: state.growth_id, id: state.data.id }
    ...
    const Savetracking = async () => {
        if (state.typedata == '' && state.type != 1) {
            showSimpleAlert(state.type == 2 ? ConstantsText.Pleaseenterheightgrowth : ConstantsText.Pleaseenterheadsizegrowth)
        }
        else if ((state.pounds === '' && state.typedata === '') && state.type == 1) {
            ...
            if (response.status === "SUCCESS") {
                ...
                if (response.code != apiConfigs.USER_UNSUBSCRIBE) {
                    if (state.type == 1) {
                        const trackEventparam = { Weight: state.pounds + '/' + state.typedata,Select_Date: state.start_date, Note:state.notes}
                        trackEvent({ event: state.data ? 'Edit_Weight_Growth' :'Add_Weight_Growth', trackEventparam })
                    }
                    else if (state.type == 2) {
                        const trackEventparam = { Height: state.typedata, Select_Date: state.start_date , Note:state.notes}
                        trackEvent({ event: state.data ? 'Edit_Height_Growth' :'Add_Height_Growth', trackEventparam })
                    }
                    else {
                        const trackEventparam = { Head_Size: state.typedata,  Select_Date: state.start_date , Note:state.notes}
                        trackEvent({ event:state.data ? 'Edit_Head_Size_Growth' : 'Add_Head_Size_Growth', trackEventparam })
                    }
                    var data = response.data
                    var isnew = state.growth_id != '' ? false : true
                    var deleteid = {}
                    if (isnew) {
                        navigation.navigate('RecentAllGrowthScreen', { child_id: state.child_id, onGoBack: true, data: data, isnew: isnew, deleteid: deleteid })
                    }
                    else {
                        route.params.onGoBack(data, isnew, deleteid);
                        navigation.goBack();
    ...
    return (
        <View style={stylesBackground.container}>
            ...
            <View style={styles.container}>
                <View>
                    {state.type == 1 ?
                        <Text style={styles.titleStyle} adjustsFontSizeToFit={true} numberOfLines={1}>{state.data ? 'Edit Weight Growth' : 'Add Weight Growth'}</Text>
                        :
                        state.type == 2 ?
                            <Text style={styles.titleStyle} adjustsFontSizeToFit={true} numberOfLines={1}>{state.data ? 'Edit Height Growth' : 'Add Height Growth'}</Text>
                            :
                            <Text style={styles.titleStyle} adjustsFontSizeToFit={true} numberOfLines={1}>{state.data ? 'Edit Head Size Growth' : 'Add Head Size Growth'}</Text>
                    }
                </View>
                ...
                >
                    ...
                    {state.type == 1 ?
                        <View style={{ flexDirection: 'row', width: deviceWidth - 34, marginTop: 30, alignItems: 'center',}}>
                            <View style={{ flexDirection: 'row', width: '50%', }}>
                                ...
                                />
                                <View style={{ justifyContent: 'center', marginTop: 10, marginStart: 5 }}>
                                    <Text style={{ color: colors.textLable, fontSize: 16, fontFamily: fonts.rubikRegular, marginTop: 10 }}>{'pounds'}</Text>
                                </View>
                            </View>
                            <View style={{ flexDirection: 'row', width: '50%', }}>
                                <TextField
                                ...
                                />
                                <View style={{ justifyContent: 'center', marginTop: 10, marginStart: 5 }}>
                                    <Text style={{ color: colors.textLable, fontSize: 16, fontFamily: fonts.rubikRegular, }}>{'ounces'}</Text>
                                </View>
                            </View>
                        </View>
                        :
                        <View style={{ flexDirection: 'row', width: deviceWidth - 34, justifyContent: 'space-between', marginTop: 30, alignItems: 'center' }}>
                            <View style={{ width: '57%' }}>
                                <TouchableWithoutFeedback onPress={() => {
                                    ...
                                    <View>
                                        <TextField
                                            key={'typedata'}
                                            textInputStyle={{ textAlign: 'center', width: '95%', marginStart: 0 }}
                                            ref={null}
                                            value={state.typedata.trim()}
                                            placeholder={''}
                                            maxLength={4}
                                            ...
                                            blurOnSubmit={true}
                                            keyboardType={'decimal-pad'}
                                            lable={state.type == 1 ? 'Weight' : state.type == 2 ? 'Height' : 'Head Size'}
                                            autoCapitalize={'none'}
                                            returnKeyType={'done'}
                                            lableStyle={{ fontFamily: fonts.rubikBold, color: colors.Blue, fontSize: 16 }}
                    ...
                    <TouchableWithoutFeedback onPress={() => {
                        ...
                        <View>
                            <TextField
                                key={'start_date'}
                                ref={null}
                                value={state.start_date != '' ? moment(state.start_date, '').format('MM/DD/YYYY') : ''}
                ...
                <CalenderModal
                    ...
                    valuesdate={state.start_date != '' ? state.start_date : moment(new Date()).format('YYYY-MM-DD')}
