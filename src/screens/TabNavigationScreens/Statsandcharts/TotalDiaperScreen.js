  ...
  const graphDataApi = async () => {
    ...
    if (response.status === "SUCCESS") {
        ...
        data.map((item) => {
          const obj =
            item.total_diaper != 0
              ...
              : { value: item.total_diaper, label: item.title };
      ...
      if (response.code == apiConfigs.USER_UNSUBSCRIBE) {
        const trackEventparam = { action: "Paywall_TotalDiaper_Screen" };
        trackEvent({ event: "Paywall_TotalDiaper_Screen", trackEventparam });
      }
    } else {
      setState((oldState) => ({
        ...oldState,
  ...
  return (
    <View style={stylesBackground.container}>
      ...
      <View style={styles.container}>
        ...
        <BottomButton
          text={"See all recent diaper updates"}
