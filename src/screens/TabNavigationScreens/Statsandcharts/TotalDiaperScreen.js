...
export default function TotalDiaperScreen({ route, navigation }) {
  ...
  const graphDataApi = async () => {
    ...
    let response = await Request.post('diaper/graph', param)
    if (response.status === 'SUCCESS') {
      state.dataGraph = []
      if (response.data.length > 0) {
        const data = response.data[0].X
        data.map(item => {
          const obj = item.total_diaper != 0 ? { value: item.total_diaper, label: item.title, topLabelComponent: () => (<Text style={{ color: colors.Blue, fontSize: 10, fontFamily: fonts.rubikBold, marginBottom: 10, width: 32, textAlign: 'center' }}>{item.total_diaper}</Text>) } : { value: item.total_diaper, label: item.title }
  ...
  const Action_continue = () => {
    action_event('See All Recent Diaper Updates')
    navigation.navigate('RecentAllDiaperScreen', { from: 'recent' })
  }
  const click_total = () => {
    action_event('Total')
    state.istype = ''
  ...
  const click_Interval = () => {
    action_event('Interval')
    state.istype = '3'
    graphDataApi()
  }
  const click_day = () => {
  ...
  const action_event = (action) => {
    ...
    trackEvent({ event: 'Total_Diaper', trackEventparam })
  }
  return (
    <View style={stylesBackground.container}>
      ...
      <View style={{ width: deviceWidth - 34, alignItems: 'center', alignSelf: 'center' }}>
        ...
        <View style={{ height: 45, justifyContent: 'center' }}>
          <Text style={styles.mainHeadertext} adjustsFontSizeToFit={true} numberOfLines={1}>{'Diaper Tracker'}</Text>
          </View>
      ...
      <View style={styles.container}>
        ...
        <View>
          <View style={{ width: deviceWidth - 34, borderBottomWidth: 1, borderTopWidth: 1, borderColor: 'rgba(34, 50, 99, 1)', height: 50, marginTop: state.isShowDay ? 3 : 48, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <TouchableWithoutFeedback onPress={() => click_BM()}>

              <View style={{}}>
                <Text style={state.istype == '1' ? styles.activeStyle : styles.inactiveStyle}>{'BM'}</Text>
              </View>
            </TouchableWithoutFeedback>
            <TouchableWithoutFeedback onPress={() => click_Wet()}>

              <View style={{}}>
                <Text style={state.istype == '2' ? styles.activeStyle : styles.inactiveStyle}>{'Wet'}</Text>
              </View>
            </TouchableWithoutFeedback>
            <TouchableWithoutFeedback onPress={() => click_Interval()}>

              <View style={{}}>
                <Text style={state.istype == '3' ? styles.activeStyle : styles.inactiveStyle}>{'Interval'}</Text>
              </View>
            </TouchableWithoutFeedback>
          </View>
        ...
        <BottomButton text={'See all recent diaper updates'} onPress={() => Action_continue()} container={{ position: 'absolute', bottom: -20 }} />
