...
const isFocused =  useIsFocused()
  ...
  const articleApi = async (values, param, checkfotter) => {
    ...
    const getallComplete =
      state.LastRecored == state.articleList.length ? false : true;
    setState((oldState) => ({
      ...oldState,
    ...
    if (response.status === "SUCCESS") {
      if (param) {
        ...
        setState((oldState) => ({
          ...
          isRefresh: false,
          pagenumber:
            response.data.total_records === state.articleList.length
              ? state.pagenumber
              : state.pagenumber + 1,
        }));
      }
  ...
  const articleApiSearch = async (values, param, checkfotter) => {
    ...
    axios
      .post(`${apiConfigs.SERVER_API_URL}${"article/list"}`, params, {
        headers: {
          Accept: "*/*",
  ...
  const fetchMore = async () => {
    const NotComplete =
      state.LastRecored != state.articleList.length ? true : false;
    if (NotComplete) {
      if (state.searchTxt == "") {
        await articleApi(false);
      }
