...
import { View, Text, StyleSheet,  BackHandler,  ScrollView,  Platform, Linking, } from 'react-native';
...
export default function ArticlesDetailesScreen({ route, navigation }) {
    ...
    const articleFavApi = async () => {
        ...
        if (response.status === 'SUCCESS') {
            if (response.code == apiConfigs.USER_UNSUBSCRIBE) {
                ...
                const trackEventparam = { name: state.articleDetails.name,Like:response.data.isLike }
    ...
    const onPress = (_, href, htmlAttribs) => {
        ...
        if (pattern.test(href)) {
            ...
            NavigationService.replaceaction('ArticlesDetailesScreen', { id: href.replace('about:///', '') })
        }
    }
    return (
        <View style={[stylesBackground.container, { justifyContent: 'center', }]}>
            ...
            <Header
                ...
                rightBtn={<FastImage source={state.isLike ? importImages.heartIcon : importImages.likeIcon} style={{ width: 40, height: 40, }} ></FastImage>}
                rightBtnOnPress={() => articleFavApi()}
            />
            <Text style={styles.mainHeader}>{'article'}</Text>
            <View style={styles.container}>
                <ScrollView showsVerticalScrollIndicator={false} bounces={false} contentContainerStyle={{ alignItems: 'center', }}>
                    <FastImage style={styles.imageStyle} source={{ uri: state.articleDetails.image }}></FastImage>
                    <View style={styles.viewStyle}>
                        <Text style={{ fontSize: 20, color: colors.Blue, fontFamily: fonts.rubikBold }}>{state.articleDetails.name}</Text>
                        <RenderHtml
                            baseStyle={{
                                fontFamily: fonts.rubikRegular, color: colors.Black,
                            }}
                            contentWidth={deviceWidth}
                            source={{ html: state.articleDetails.description }}
                            renderersProps={{ a: { onPress, } }}
                            systemFonts={[...defaultSystemFonts, 'Rubik-Bold', 'Rubik-Regular']}
                            tagsStyles={{
                                p: { marginTop: 0 }
                            }}
                        />
            ...
            <BottomButton text={'share'} container={{ width: deviceWidth - 22, alignSelf: 'center', position: 'absolute', bottom: -10 }} textstyle={{ fontFamily: fonts.rubikBold }} onPress={() => onShare()} />
