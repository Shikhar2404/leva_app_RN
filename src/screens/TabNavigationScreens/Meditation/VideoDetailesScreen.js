...
export default function VideoDetailesScreen({ route, navigation }) {
    ...
    const goBackNav = () => {
        ...
        setTimeout(async() => {
            if (state.videoDetails.type == 1) {
                ...
                const trackEventparam = { name: state.videoDetails.name, Time_in_Video:AppPlayer.secondsToHHMMSS(currentTime) }
    ...
    return (
        <View style={[stylesBackground.container, { backgroundColor: state.videoDetails ? colors.Black : colors.pink }]}>
            {state.videoDetails && Object.keys(state.videoDetails).length === 0 && Object.getPrototypeOf(state.videoDetails) === Object.prototype ? null :
                <View style={{ width: '100%', height: '100%', }}>
                    ...
                    {state.videoDetails.type == 2 ?
                        ...
                        <Vimeo
                            ...
                            params={state.isSubscribe ? 'api=1&autoplay=1&transparent=0' : 'api=1&autoplay=0&transparent=0'}
            ...
            {state.isSubscribe ?
                <SubscriptionModalView
                    style={{ height: '100%', backgroundColor: colors.White, opacity: 0.7 }}
