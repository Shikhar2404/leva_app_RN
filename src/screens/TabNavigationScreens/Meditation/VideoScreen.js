...
export default function VideoScreen({ route, navigation }) {
        ...
        <TouchableWithoutFeedback onPress={() => click_video(index, item)}>
          <View style={styles.listViewStyle}>
            <View style={styles.listImageStyle}>
              <FastImage
                source={{ uri: item.image }} style={[styles.listImageStyle,]}></FastImage>
            </View>
            <View style={{
              position: 'absolute',
              height: 105,
  ...
  const videoApi = async (values, param, checkfotter) => {
    const getallComplete = state.LastRecored == state.videoList.length ? false : true
    setState(oldState => ({
      ...oldState,
    ...
    if (response.status === 'SUCCESS') {
      if (param) {
        ...
        setState(oldState => ({
          ...oldState,
          videoList: state.isRefresh ? response.data.videos : JSFunctionUtils.uniqueArray(state.videoList, response.data.videos, "video_id"),
          isModalVisible: false,
          isModalFooterVisible: false,
          LastRecored: response.data.total_records,
          isRefresh: false,
          pagenumber: response.data.total_records === state.videoList.length ? state.pagenumber : state.pagenumber + 1,
  ...
  const videoApiSearch = async (values, param, checkfotter) => {
    ...
    axios.post(`${apiConfigs.SERVER_API_URL}${'video/list'}`, params, {
      headers: {
        'Accept': '*/*',
  ...
  const fetchMore = () => {
    const NotComplete = state.LastRecored != state.videoList.length ? true : false
    ...
    <View style={stylesBackground.container}>
      ...
      <View style={styles.container}>
        <FlatList
          ...
          contentContainerStyle={state.videoList.length > 0 ? {} : { flexGrow: 1, justifyContent: 'center', alignItems: 'center', }}
