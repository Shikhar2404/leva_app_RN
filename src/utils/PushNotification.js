...
export default function PushNotifications({ route, navigation }) {
      ...
      messaging().onMessage(async remoteMessage => {
         if (appState == "active") {
            ...
            showMessage({
               ...
               autoHide: true,
               titleStyle: { color: colors.Blue, fontSize: 16, marginHorizontal: 10, fontFamily: fonts.rubikBold },
               textStyle: { color: colors.Blue, fontSize: 14, marginHorizontal: 10, fontFamily: fonts.rubikRegular },
   ...
   const handleNotificationRedirection = async (notification) => {
      if (notification.data.type == '1') {
         ...
         setTimeout(() => {
            NavigationService.replaceactionNotification('ArticlesDetailesScreen', { id: notification.data.article_id })
         }, 500);
      }
         ...
         setTimeout(() => {
            NavigationService.replaceactionNotification('MeditationDetailesScreen', { meditationsData: [], index: -1, isNotification: notification.data.meditation_id, })
         }, 600);
      }
      else if (notification.data.type == '6') {
         setTimeout(() => {
            NavigationService.replaceactionNotification('VideoDetailesScreen', { id: notification.data.video_id })
         }, 500);
      }
         ...
         setTimeout(() => {
            NavigationService.navigatewithparamNotification1('TotalGrowthScreen', { child_id: '' })
         }, 500);
      }
